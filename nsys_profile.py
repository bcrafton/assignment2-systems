...
def fake_train(N, vocab_size, batch_size, context_length):
  ...
  for _ in range(N):
    ...
    T1 = time.time()
    # torch.cuda.nvtx.range_push('Forward Pass') # or use a context manager if available
    p = m(x)
    # torch.cuda.nvtx.range_pop()
    
    T2 = time.time()
    # torch.cuda.nvtx.range_push('Backward Pass') # or use a context manager if available
    loss = cross_entropy(p, y)
    loss = loss.backward()
    # torch.cuda.nvtx.range_pop()
    
    T3 = time.time()
    # torch.cuda.nvtx.range_push('Optimizer Pass') # or use a context manager if available
    optimizer.step()
    # torch.cuda.nvtx.range_pop()
...
for config in [small]:
  for context_length in [1024]:
    ...
    try:
      ...
      # run 1 steps
      inf, grad, update = fake_train(N=10, vocab_size=config['vocab_size'], batch_size=1, context_length=context_length)
